...
export class PromisePool<
  T,
  ShouldUseCorrespondingResults extends boolean = false,
> {
  /**
  ...
  static withConcurrency(concurrency: number): PromisePool<unknown> {
    return new PromisePool().withConcurrency(concurrency);
  }
  ...
  static withTaskTimeout(timeout: number): PromisePool<unknown> {
    return new PromisePool().withTaskTimeout(timeout);
  }
  ...
  for<ItemType>(items: SomeIterable<ItemType>): PromisePool<ItemType> {
    const pool = new PromisePool<ItemType>(items).withConcurrency(
      this.concurrency,
    );
  ...
  static for<T>(items: SomeIterable<T>): PromisePool<T> {
    return new PromisePool<T>().for(items);
  }
  ...
  async process<ResultType, ErrorType = any>(
    ...
    ReturnValue<
      T,
      ShouldUseCorrespondingResults extends true
        ? ResultType | symbol
        : ResultType,
      ErrorType
    >
