...
export class PromisePoolExecutor<T, R>
  implements UsesConcurrency, Stoppable, Statistics<T>
{
  /**
  ...
  useConcurrency(concurrency: number): this {
    if (!this.isValidConcurrency(concurrency)) {
      throw ValidationError.createFrom(
        `"concurrency" must be a number, 1 or up. Received "${concurrency}" (${typeof concurrency})`,
      );
    }
  ...
  /**
   * Expose validation for testing purposes.
   */
  validate(): this {
  ...
  validateInputs(): this {
    ...
    if (!(timeout == null || (typeof timeout === "number" && timeout >= 0))) {
      throw ValidationError.createFrom(
        `"timeout" must be undefined or a number. A number must be 0 or up. Received "${String(timeout)}" (${typeof timeout})`,
      );
    }
    ...
    this.onTaskStartedHandlers.forEach((handler) => {
      if (handler && typeof handler !== "function") {
        throw ValidationError.createFrom(
          `The onTaskStarted handler must be a function. Received "${typeof handler}"`,
        );
      }
    });
    ...
    this.onTaskFinishedHandlers.forEach((handler) => {
      if (handler && typeof handler !== "function") {
        throw ValidationError.createFrom(
          `The error handler must be a function. Received "${typeof handler}"`,
        );
      }
    });
  ...
  async process(): Promise<ReturnValue<T, R>> {
    ...
    for await (const item of this.items()) {
      if (this.isStopped()) {
        stoppedError = new StopThePromisePoolError();
        break;
      }
  ...
  async createTaskFor(item: T, index: number): Promise<any> {
    ...
    // Wrap the handler to catch timeout and set failed symbol
    return Promise.race([
      this.handler(item, index, this),
      timer().catch((err) => {
        // If timeout, throw error to be handled in catch
        throw err;
      }),
    ]).finally(canceller);
  }
  ...
  private createTaskTimeout(item: T): [() => Promise<void>, () => void] {
    ...
    const timer: () => Promise<void> = async () =>
      new Promise<void>((_resolve, reject) => {
        timerId = setTimeout(() => {
          reject(
            new PromisePoolError(
              `Task in promise pool timed out after ${this.taskTimeout() as number}ms`,
              item,
            ),
          );
        }, this.taskTimeout());
  ...
  async handleErrorFor(error: Error, item: T, index: number): Promise<void> {
    if (this.shouldUseCorrespondingResults()) {
      ...
      this.results().push(PromisePool.failed);
    }
