...
test("ensures concurrency is a number", async () => {
  ...
  expect(await pool.withConcurrency(1).process(fn)).toEqual({
    errors: [],
    results: [],
  });
  expect(await pool.withConcurrency(Infinity).process(fn)).toEqual({
    errors: [],
    results: [],
  });

  await expect(pool.withConcurrency(0).process(fn)).rejects.toThrow(
    ValidationError,
  );
  await expect(pool.withConcurrency(-1).process(fn)).rejects.toThrow(
    ValidationError,
  );
  await expect(pool.withConcurrency(null).process(fn)).rejects.toThrow(
    ValidationError,
  );
});
...
test("ensures timeout is a valid number", async () => {
  ...
  await expect(pool.withTaskTimeout(-1).process(fn)).rejects.toThrow(
    ValidationError,
  );
  await expect(pool.withTaskTimeout("-1").process(fn)).rejects.toThrow(
    ValidationError,
...
test("ensures the items are an array", async () => {
  ...
  await expect(await pool.for([]).process(fn)).toEqual({
    errors: [],
    results: [],
  });
});
...
test("fails with string", async () => {
  ...
  expect(errors.every((error) => error.message === "failed")).toBe(true);
});
...
test("fails with Error (and stacktrace)", async () => {
  ...
  expect(errors.every((error) => error.message === "failing")).toBe(true);

  expect(errors.every((error) => typeof error.stack === "string")).toBe(true);
});
...
test("fails when not passing a function as an onTaskStarted callback", async () => {
  const pool = await PromisePool.for([1, 2, 3]).onTaskStarted("non-function");
...
test("fails when not passing a function as an onTaskFinished callback", async () => {
  const pool = await PromisePool.for([1, 2, 3]).onTaskFinished("non-function");
...
test("useCorrespondingResults keeps results in order", async () => {
  ...
  const { results } = await PromisePool.for(timeouts)
...
test("can timeout long-running handlers", async () => {
  ...
  const { results, errors } = await PromisePool.withTaskTimeout(10)
    .for(timers)
    .process(async (timer) => {
